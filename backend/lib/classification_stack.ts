...
export class ForestClassificationStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const geeCredentialsFileParam = new cdk.CfnParameter(this, "GeeCredentialsFile", {
      type: "String",
      description:
        "Name of the Google Earth Engine credentials file in the S3 bucket (e.g., credentials/custom-gee-credentials.json)",
    })
    ...
    // Frontend Build Bucket for Amplify deployment (similar to Catholic Charities)
    const frontendBucket = new s3.Bucket(this, "FrontendBuildBucket", {
      bucketName: `${bucketName}-builds-${this.account}-${this.region}`.substring(0, 63),
      removalPolicy: cdk.RemovalPolicy.DESTROY,
    ...
    // Grant Amplify service access to the build bucket
...
`),
    ...
    // EventBridge rule to trigger Amplify deployment when build is uploaded
